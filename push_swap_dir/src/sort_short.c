...
static void		ft_sort_three(t_data *data)
{
	...
	A_ST < A_ND && A_ND > A_RD && A_ST < A_RD
	? execute_sa(data) & execute_ra(data) : 0;
...
void			ft_sort_five(t_data *data)
{
	while (ft_lstlen(data->stack_a) != 2)
		(CURRENT_A_INDEX != data->len && CURRENT_A_INDEX != 0)
	...
	B_ST > B_ND && B_ND < B_RD && B_ST > B_RD
	? execute_sb(data) & execute_rb(data) : 0;
